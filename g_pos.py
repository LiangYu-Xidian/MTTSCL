...
def read_full_data():
    data_df = pd.read_csv("./data/g_pos.csv")
    ...
    y = label_encode(data_df["label"].values.tolist(), ["C","CM","E","CW"])
...
if __name__ == "__main__":
    ...
    criterion = nn.CrossEntropyLoss(weight=weights1, ignore_index=len(config.classes_g_pos)).to(config.device)
    ...
    for i, (train_idx, test_idx) in enumerate(kf.split(x,y)):
        ...
        model = models.SingleTask_g_pos(config)
